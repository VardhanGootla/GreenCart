...
const ProductDetails = () => {
   ...
   const {id} = useParams()    
    ...
    },[products, product])   // product bhi dependency mai add kiya
...
<div className="flex flex-col items-center w-max">
    ...
    <div className="w-20 h-0.5 bg-primary rounded-full mt-2"></div>
               ...
               <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 ga[-3 md:gap-16 lg:grid-cols-5 mt-6 w-full">
                {relatedProducts.filter(() => product.inStock).map((product,index)=>(
                    <ProductCard key={index} product={product} />
                ))}
               </div>
               <button onClick={() =>{navigate('/products'); scrollTo(0,0)}} className="mx-auto cursor-pointer px-12 my-16 py-2.5 border text-primary bover:bg-primary/10 transition">See More</button>
            </div>
        </div>
    );
};
